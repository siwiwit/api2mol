...
module ECoreTestMetamodels

  def setup_simple_classifiers_metamodel
    package = ECore::EPackage.new(:name => 'SimpleTest')
    package.nsURI = 'http://simple/test'
    package.nsPrefix = 'test'
    ...
    a = ECore::EClass.new(:name => 'A')
    b = ECore::EClass.new(:name => 'B')
    b.eSuperTypes << a
    d = ECore::EDataType.new(:name => 'MyDataType')
    e = ECore::EEnum.new(:name => 'MyEnumeration')
    e.eLiterals << ECore::EEnumLiteral.new(:name => 'Literal1')
    e.eLiterals << ECore::EEnumLiteral.new(:name => 'Literal2')
    
    [root, a, b, d, e].each { |c| 
      package.eClassifiers << c
    }
    
    @simple_metamodel_Root = root
    @simple_metamodel_A = a
    @simple_metamodel_B = b
    @simple_metamodel = RMOF::Model.new(package.nsURI, [package])
  end
  
  def setup_class_metamodel
    package = ECore::EPackage.new(:name => 'ClassModel')
    package.nsURI = 'http://class/test'
    package.nsPrefix = 'class_test'
    
    named_element = ECore::EClass.new(:name => 'NamedElement') do |named|
      named.eStructuralFeatures << ECore::EAttribute.new(:name => 'name', :eType => ECore::EString)
    end
    classifier = ECore::EClass.new(:name => 'Classifier') do |classifier|
      classifier.eSuperTypes << named_element           
    end
    klass = ECore::EClass.new(:name => 'Class')
    klass.eSuperTypes << classifier
    ptype = ECore::EClass.new(:name => 'PrimitiveType')
    ptype.eSuperTypes << classifier
    attribute = ECore::EClass.new(:name => 'Attribute') do |a|
      a.eStructuralFeatures << ECore::EAttribute.new(:name => 'name', :eType => ECore::EString)
    end
    
    klass.eStructuralFeatures << ECore::EReference.new(:name => 'attributes', :eType => attribute)
    
    [named_element, classifier, klass, ptype, attribute].each { |c| 
      package.eClassifiers << c
    }        
    ...
    @class_Class      = klass
    @class_PType      = ptype
    @class_metamodel = RMOF::Model.new(package.nsURI, [package])  
  end
  ...
  def setup_shop_metamodel
    ...
    supermarket = ECore::EClass.new(:name => 'Supermarket') do |supermarket|
      ...
      supermarket.eStructuralFeatures << ECore::EReference.new(:name => 'bakerySection', :upperBound => 1,                                                             :eType => bakery, :containment => true)
    end 
    ...
    @model = RMOF::Model.new(package.nsURI, [package])
